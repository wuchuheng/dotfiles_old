...
function recieveTrash() {
    ...
    if [ -e ~/$target ]; then
        if [[ 
            ...
            $(file ~/$target | awk '{print $NF}' | grep 'dotfiles/$target') != "dotfiles/$target" ]]; then
            mv -f ~/$target ./trash/
        fi
    fi
}
...
function install() {
    ...
    #安装nvim 配置
    if [[ hasNvim && ! -e ${XDG_DATA_HOME:-$HOME/.local/share}/nvim/site/autoload/plug.vim ]]; then
        sh -c 'curl -fLo "${XDG_DATA_HOME:-$HOME/.local/share}"/nvim/site/autoload/plug.vim --create-dirs https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim'
        ln -s $(pwd)/.vimrc ~/.config/nvim/init.vim
    elif [[ hasVim && ! -e ./.vim/authload/plug.vim ]]; then
